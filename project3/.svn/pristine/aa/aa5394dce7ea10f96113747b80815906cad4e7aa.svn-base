        ...
        for state in self.mdp.getStates():
            ...
            for action in self.mdp.getPossibleActions(state):
              	...
              	for nextState, prob in self.mdp.getTransitionStatesAndProbs(state,action):
                	valuesum += prob*((self.mdp.getReward(state,action,nextState)+self.discount*self.getValue(nextState)))
              	if bestValue <= valuesum:
                	bestValue = valuesum
                	temp[state] = bestValue
    	self.values = temp
