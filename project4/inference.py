...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()  # print( self.getJailPosition() )
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # essentially the equation: P( g_t+1 | g_t ) = \SUM{ P( g_t+1 | g_t )*P( g_t | d_1:t ) }
        pacmanPosition = gameState.getPacmanPosition()
        new_beliefs = util.Counter()

        for new_p in self.legalPositions:
            for old_p in self.legalPositions:
                new_pos_dist = self.getPositionDistribution(
                    self.setGhostPosition(gameState, old_p) )
                new_beliefs[new_p] += new_pos_dist[new_p] * self.beliefs[old_p]
...
class ParticleFilter(InferenceModule):
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # collapse all samples into a distribution (i.e. a counter)
        distr = util.Counter()
        for si in self.particles:
            distr[si] += 1
        distr.normalize()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in range(self.numParticles):
            new_particle = [
                random.choice(self.legalPositions) for j in range(self.numGhosts)
                ]
            self.particles.append( tuple(new_particle) );
        # print( "initial particles: ", self.particles )
        return
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range( len(newParticle) ):
                new_p_distr = getPositionDistributionForGhost(
                    setGhostPositions(gameState, oldParticle),
                    i, self.ghostAgents[i]
                )
                newParticle[i] = util.sampleFromCounter(new_p_distr)
    ...
    def observeState(self, gameState):
        ...
        for i in range( self.numParticles ):
            ...
            # form a new particle which doesn't contain any ghost that's eaten
            new_particle = []
            for gi in range( len(noisyDistances) ):
                if noisyDistances[gi] != None:
                    new_particle.append(particle[gi])
                else:
                    new_particle.append( self.getJailPosition(gi) )
            # for remaining ghosts, calculate its weight
            w = 1.0
            for gi in range(self.numGhosts):
                if noisyDistances[gi] == None:
                    continue
                d = util.manhattanDistance(particle[gi], pacmanPosition)
                w *= emissionModels[gi][d]
            if w > 0.0:
                all_zero = False
            wts[ tuple(new_particle) ] += w
        wts.normalize()
        # print( "wts: ", wts )
        # print( wts.totalCount() )
