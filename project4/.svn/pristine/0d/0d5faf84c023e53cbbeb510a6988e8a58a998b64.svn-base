...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for distribution in livingGhostPositionDistributions:
    	if(self.distancer.getDistance(pacmanPosition, distribution.argMax()) < Distance):
    		Distance =  self.distancer.getDistance(pacmanPosition, distribution.argMax())
    		Position = distribution.argMax()
    d = float("inf")
    ba = None
    for action in legal:
    	if(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition,action),distribution.argMax()) < d):
    		d = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition,action),distribution.argMax())
    		ba = action
    return ba
