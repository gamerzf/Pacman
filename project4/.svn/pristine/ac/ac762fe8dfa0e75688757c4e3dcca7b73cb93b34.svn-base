...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:#a ghost is captured by Pacman
      newParticles = []
      ...
      for particle in self.particles:
        ...
        newParticleWeight = emissionModel[trueDistance]
        particleWeights[particle] += newParticleWeight
      if particleWeights.totalCount() == 0:#all particles receive 0 weight
        self.initializeUniformly(gameState)
      else:
        particleWeights.normalize()
        for i in range(self.numParticles):
          self.particles[i] = util.sample(particleWeights)
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    self.particles = []
    for i in range(self.numParticles):
      new_particle = [random.choice(self.legalPositions) for j in range(self.numGhosts)]
  ...
  def observeState(self, gameState):
    ...
    for j in range(self.numParticles):
      ...
      for i in range (len(noisyDistances)):
        if noisyDistances[i] == None:
          particles.append(self.getJailPosition(i))
        else:
      ...
      for i in range(self.numGhosts):
        if noisyDistances[i] == None:
          ...
          trueDistance = util.manhattanDistance(list(particle)[i], pacmanPosition)
          newParticleWeight *= emissionModels[i][trueDistance]
      particleWeights[tuple(particles)] += newParticleWeight
    ...
    if particleWeights.totalCount() == 0:
      ...
      for j in range(len(self.particles)):
        for i in range(self.numGhosts):
          if noisyDistances[i] == None:
            listParticle = list(self.particles[j])
            listParticle[i] = self.getJailPosition(i)
            self.particles[j] = tuple(listParticle)
